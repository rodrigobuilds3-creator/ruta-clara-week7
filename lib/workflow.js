...
export function transitionCase(item, action, now, note = "") {
  ...
  if (action === "verify" && item.status === "submitted") return {
    ...
    history: [...item.history, { at: now, actor: "Verificador independiente (demo)", action: "Riesgo verificado; pago elegible" }],
  };
  if (action === "reject" && item.status === "submitted" && reason.length >= 12) return {
    ...item, status: "rejected", paymentEligible: false,
    history: [...item.history, { at: now, actor: "Verificador independiente (demo)", action: `Rechazado: ${reason}` }],
  };
  if (action === "close" && item.status === "verified" && reason.length >= 12) return {
    ...item, status: "closed", remedy: reason,
    history: [...item.history, { at: now, actor: "Autoridad / concesionario (demo)", action: `Cierre firmado en simulación: ${reason}` }],
  };
...
export function countUnresolved(items) { return items.filter(item => item.status !== "closed" && item.status !== "rejected").length; }
